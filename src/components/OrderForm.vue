...
const orderRules = reactive<FormRules>({
  ...
  discount_strategy: [{ required: true, message: '此字段为必填项', trigger: 'change' }],
  pay_strategy: [{ required: true, message: '此字段为必填项', trigger: 'change' }],
})
  ...
  formEl.validate((valid) => {
    ...
    const r = request({
      ...
      data: {
        ...
        discount_strategy: orderForm.discount_strategy,
        pay_strategy: orderForm.pay_strategy
      }
    })
...
<template>
  ...
  <el-form ref="orderFormRef" :model="orderForm" :rules="orderRules" label-width="120px">
    ...
    <el-form-item label="优惠方式" prop="discount_strategy">
      <el-select v-model="orderForm.discount_strategy">
        <el-option label="积分抵扣" value="积分抵扣" />
        <el-option label="不使用积分" value="不使用积分" />
      </el-select>
    </el-form-item>
    <el-form-item label="支付方式" prop="pay_strategy">
      <el-select v-model="orderForm.pay_strategy">
